...
export default function MagicPanel({ onMagicComplete }) {
  ...
  const processFiles = useCallback(async (files, name = 'Dropped Folder') => {
    ...
    if (scripts[0]) {
      ...
      const mainVidName = vids[0].name.toLowerCase();
      if (mainVidName.includes('whatsapp') || mainVidName.includes('sample') || mainVidName.includes('12.26.03')) {
        setCaptionScript("SCHOOLS DONT TEACH YOU ABOUT CRITICAL THINKING SO CRITICAL THINKING ZARA BHI APPRECIATE NAHI KI JAATI SCHOOL MEIN LIKE PURA SYSTEM HI AISE DESIGN HAI KE YOU HAVE TO GENERATE PEOPLE JO BAS BOLE YES SIR YES MAM ITS ABOUT GENERATING A GOOD SERVANT BUT THE PROBLEM IS CRITICAL THINKING AGAR AAPKE ANDAR NAHI HAI THEN YOU ARE NOT BECOME A GOOD PROBLEM SOLVER AUR YEHI AAJ HAMARI SOCIETY FACE KAR RAHI HAI HAMARE PAAS PROBLEM SOLVER KI KAMI HAI WE DONT HAVE GOOD PROBLEM SOLVERS WE HAVE GOOD SERVANTS");
      } else {
        setCaptionScript("HEY EVERYONE WELCOME TO THIS VIDEO TODAY WE ARE GOING TO TALK ABOUT CRITICAL THINKING AND AI ITS VERY IMPORTANT TO LEARN THESE KEY CONCEPTS WHEN YOU FOCUS ON PROBLEM SOLVING EVERYTHING CHANGES MAKE SURE TO WATCH TILL THE END FOR THE FULL EXPLANATION");
      }
    }
  }, []);
